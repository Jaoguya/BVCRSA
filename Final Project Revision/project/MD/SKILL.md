...
> ## 🛑 EVERY CROSS-SCHEME RESULT ON DISK IS STALE
>
> The AWS workers finished **2026-08-16 22:32**. `baselines.py`,
> `vckase.py`, `latt_ibeks.py`, `trinity.py` and `Attribute-based.py`
> were all rewritten **2026-08-17 17:38** — *after* those runs. So every
> number in `logs/` and in the AWS-produced CSVs was generated by the
> **old, fake baselines** (VC-KASE's "pairing" = `pow(g1*g2, 3, p)`;
> Latt-IBEKS's trapdoor = `np.poly` padded to a fixed 5 roots).
>
> **Experiments 01, 02, 03 and 04 must all be re-run.** All four consume
> `baselines.py`. Exp 05–08 are ablations against BVCRSA itself, never
> touch `baselines.py`, and still stand.
>
> Two further contaminations of the CSV directory:
> - `exp01_trapdoor_gen.csv` and `exp02_query_processing.csv` carry
>   `env_host=jzguyr.local` — a **local smoke run at N=300, d=1 only**
>   overwrote W1's and W2's full-scale AWS output. Those AWS results now
>   survive **only** in `logs/`. This is exactly the name-collision
>   `AWS/README.md` §"Collecting" warns about.
> - `exp09_sensor_side_cost.csv` has `env_host=DESKTOP-9RLOO3O` — a
>   Windows box, not the Raspberry Pi R1-C6 requires.
>
> **There are currently no publishable cross-scheme numbers.**
